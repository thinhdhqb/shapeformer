...
parser.add_argument('--aug_ratio', default=2, type=int, help='Number of augmented copies to create per instance')
...
def augment_time_series_with_noise(time_series, shapelets_info, shapelet_std=0.1, non_shapelet_std=0.5, num_copies=2):
    ...
    for si in shapelets_info:
        orig_ts_idx = int(si[0])  # original time series index
        dim = int(si[5])     
        start = int(si[1])   
        end = int(si[2])     
    ...
    # Add noise to non-shapelet regions
...
if __name__ == '__main__':
    ...
    for problem in list_dataset_name[config['dataset_pos']:config['dataset_pos']+1]:  # for loop on the all datasets in "data_dir" directory
        ...
        print("All_train_data shape before augmentation: ", Data['All_train_data'].shape)
        Data['All_train_data'] = np.concatenate([Data['All_train_data'], augmented_data], axis=0)
        Data['All_train_label'] = np.tile(Data['All_train_label'], args.aug_ratio + 1)
